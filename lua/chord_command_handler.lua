...
--[[
一个新点子！！！！！！
我们不如判定，如果没有在并击状态而就先按下了空格，那就直接空格上屏rejected，
任何用作空格的键都这样处理，比如副空格的引号键
但是唯独问题是如果在并击一简字的时候按到了怎么办……
那就按距离上次的时间来算吧，如果距离上次并击输出中文的时间很长，那这次空格很有可能是单独空格；如果一直在并击中文，那么这次空格很有可能是空格并选
但是还是要先观察一下击键事件相关）
]]--
